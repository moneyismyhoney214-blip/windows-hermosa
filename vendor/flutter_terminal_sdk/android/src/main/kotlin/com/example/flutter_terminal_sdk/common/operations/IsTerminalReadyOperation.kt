...
class IsTerminalReadyOperation(provider: NearpayProvider) : BaseOperation(provider) {
    override fun run(filter: ArgsFilter, response: (Map<String, Any>) -> Unit) {

        val terminalUUID = filter.getString("terminalUUID") ?: return response(
            ResponseHandler.error("MISSING_terminalUUID", "jwt is required")
        )
        ...
        Timber.tag("IsTerminalReadyOperation")
            .d("Checking if terminal is ready for UUID: $terminalUUID")
