...
class _OrdersScreenState extends State<OrdersScreen> {
  ...
  Future<Map<String, dynamic>> _loadBookingsForOrderSearch({
    ...
    // Backend search can mismatch between booking.id and order.id.
    // Fallback: scan today's pages and match locally on order.id first.
    const maxPagesToScan = 8;
    final localMatches = <Booking>[];

    for (var page = 1; page <= maxPagesToScan; page++) {
      final response = await _orderService.getBookings(
        page: page,
        dateFrom: dateFromStr,
        dateTo: dateToStr,
        status: _selectedStatus == 'all' ? null : _selectedStatus,
      );
      final pageBookings = _bookingsFromResponse(response);
      if (pageBookings.isEmpty) break;
      ...
      if (totalCount != null && page * 20 >= totalCount) break;
      if (pageBookings.length < 20) break;
    }

    // Last fallback for explicit order-id search:
    // Query without date filters because many environments store historical
    // orders outside "today" and backend search might only match globally.
    for (var page = 1; page <= maxPagesToScan; page++) {
      final response = await _orderService.getBookings(
        page: page,
        search: orderIdSearch,
  ...
  Future<void> _showCreateInvoiceDialog(Booking booking) async {
    ...
    double resolveInvoiceTotal() {
      ...
      var subtotal = booking.total;
      if (subtotal <= 0) subtotal = 0;
      var total = subtotal + booking.tax - booking.discount;
  ...
  Future<void> _processDeferredInvoice(
    ...
    try {
      ...
      try {
        invoiceResponse = await _orderService.createInvoiceMultipart(payload);
      ...
      // Auto-print CASHIER receipt only (NO kitchen — food already prepared)
      ...
      unawaited(_printCashierReceiptForOrder(
        invoiceId: resolvedInvoiceId.isNotEmpty ? resolvedInvoiceId : booking.id.toString(),
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return _buildCleanBookingCard(booking);
        },
      ),
    );
  }
  ...
  double _bookingGrandTotal(Booking booking) {
    final raw = booking.raw;
    double base;
    // 1. Try grand_total directly from API
    ...
    if (gt != null && gt > 0) {
      ...
      // 3. Calculate from meals: sum prices and add 15% tax
      double sum = 0;
      for (final meal in booking.meals) {
        sum += meal.total > 0 ? meal.total : (meal.unitPrice * meal.quantity);
      }
      base = sum > 0 ? sum * 1.15 : booking.total;
    } else if (booking.total > 0) {
      // 4. Fallback: total * 1.15
      base = booking.total * 1.15;
    } else {
      base = booking.total;
    }
    // Subtract locally tracked refund amounts (pre-tax → multiply by 1.15)
    final refundedPreTax = _bookingRefundedAmounts[booking.id];
    if (refundedPreTax != null && refundedPreTax > 0) {
      base = (base - refundedPreTax * 1.15).clamp(0.0, double.infinity);
    }
    return base;
  }
